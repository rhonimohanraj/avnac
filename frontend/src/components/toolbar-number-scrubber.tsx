...
export default function ToolbarNumberScrubber({
  ...
  if (editing) {
    return (
      <input
        ...
        aria-label={editTitle ?? ariaLabel}
      />
    )
  }
  ...
  return (
    <div
      ...
      title={title}
      className={[
        'flex h-8 cursor-ew-resize select-none items-center justify-center rounded-lg border border-black/10 bg-white px-2 text-xs tabular-nums text-neutral-900 touch-none hover:border-black/18',
        icon ? 'min-w-[4.5rem] gap-1.5' : 'min-w-[2.75rem]',
      ].join(' ')}
