  ...
  return children.map(child => {
    ...
    if (objectSupportsCornerRadius(next)) {
      return setObjectCornerRadius(
        next,
        Math.min(
          getObjectCornerRadius(next) * Math.max(scaleX, scaleY),
          maxCornerRadiusForObject(next),
        ),
      )
    }
    return next
...
export function isPerfectShapeObject(obj: SceneObject): boolean {
  return (
    obj.type === 'rect' || obj.type === 'ellipse' || obj.type === 'polygon' || obj.type === 'star'
  )
}
...
function normalizedImageCrop(image: SceneImage): SceneImage['crop'] {
  const naturalWidth = Math.max(IMAGE_MIN_CROP_SIZE, image.naturalWidth || image.width)
  const naturalHeight = Math.max(IMAGE_MIN_CROP_SIZE, image.naturalHeight || image.height)
  const width = clampNumber(image.crop.width || naturalWidth, IMAGE_MIN_CROP_SIZE, naturalWidth)
  const height = clampNumber(image.crop.height || naturalHeight, IMAGE_MIN_CROP_SIZE, naturalHeight)
  return {
...
function fitImageCropToAspect(
  ...
  const maxWidth = safeAspect >= naturalAspect ? naturalWidth : naturalHeight * safeAspect
  const maxHeight = safeAspect >= naturalAspect ? naturalWidth / safeAspect : naturalHeight
  ...
  width = clampNumber(width, IMAGE_MIN_CROP_SIZE, Math.max(IMAGE_MIN_CROP_SIZE, maxWidth))
  height = clampNumber(height, IMAGE_MIN_CROP_SIZE, Math.max(IMAGE_MIN_CROP_SIZE, maxHeight))
...
function cropImageFromSideHandle(
  ...
  if (handle === 'e' || handle === 'w') {
    ...
    let x = crop.x
    if (centeredScaling) {
      x = clampNumber(cropCenterX - width / 2, 0, Math.max(0, naturalWidth - width))
      width = Math.min(width, naturalWidth)
      next.x =
        Math.abs(width - requestedWidth) < 0.001 ? box.x : displayCenterX - (width * scale) / 2
    } else if (handle === 'w') {
      const cropRight = crop.x + crop.width
      width = Math.min(width, cropRight)
      x = cropRight - width
      next.x = Math.abs(width - requestedWidth) < 0.001 ? box.x : displayRight - width * scale
    } else {
  ...
  if (handle === 'n' || handle === 's') {
    const scale = Math.max(IMAGE_MIN_SCALE, image.width / Math.max(IMAGE_MIN_CROP_SIZE, crop.width))
    const cropCenterY = crop.y + crop.height / 2
    const requestedHeight = box.height / scale
    let height = clampNumber(requestedHeight, IMAGE_MIN_CROP_SIZE, naturalHeight)
    let y = crop.y
    if (centeredScaling) {
      y = clampNumber(cropCenterY - height / 2, 0, Math.max(0, naturalHeight - height))
      height = Math.min(height, naturalHeight)
      next.y =
        Math.abs(height - requestedHeight) < 0.001 ? box.y : displayCenterY - (height * scale) / 2
    } else if (handle === 'n') {
      const cropBottom = crop.y + crop.height
      height = Math.min(height, cropBottom)
      y = cropBottom - height
      next.y = Math.abs(height - requestedHeight) < 0.001 ? box.y : displayBottom - height * scale
    } else {
...
export function reorderTopLevelObjects(
  ...
  if (kind === 'front' || kind === 'back') {
    ...
    const next = kind === 'front' ? [...rest, ...picked] : [...picked, ...rest]
...
export function resizeObjectWithBox(
  ...
  if (next.type === 'text') {
    const initialText = opts?.initial?.type === 'text' ? opts.initial : next
  ...
  if (next.type === 'image') {
    const initialImage = opts?.initial?.type === 'image' ? opts.initial : next
